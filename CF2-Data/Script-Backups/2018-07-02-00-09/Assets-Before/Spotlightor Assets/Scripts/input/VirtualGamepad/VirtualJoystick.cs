...
public class VirtualJoystick : VirtualJoystickBase
{
	...
	{
		if (!IsPressed) {
			if (Input.GetMouseButton (0) && IsInJoystickScreenArea (Input.mousePosition)) {
				IsPressed = true;
				SetupJoystickAt (new Vector2 (Input.mousePosition.x, Input.mousePosition.y));
			}
		} else {
			if (Input.GetMouseButton (0) == false)
				IsPressed = false;
			else {
				fingerPosition = new Vector2 (Input.mousePosition.x, Input.mousePosition.y);
				UpdateStrength ();
			}
		}
	}
