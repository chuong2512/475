...
public class ObjectDirectionalDragger : FunctionalMonoBehaviour
{
	...
	public float Progress {
		get { return MaxDistance != 0 ? Distance / MaxDistance : 0;}
		set {
	...
	private bool IsFingerDown {
		get {
			if (SystemInfo.deviceType == DeviceType.Handheld)
				return ControlFreak2.CF2Input.touchCount > 0 && ControlFreak2.CF2Input.touches [0].phase == TouchPhase.Began;
			else
	...
	private bool IsFingerUp {
		get {
			if (SystemInfo.deviceType == DeviceType.Handheld)
				return ControlFreak2.CF2Input.touchCount == 0;
			else
				return ControlFreak2.CF2Input.GetMouseButtonUp (0);
		}
	}
	...
	private Vector2 CurrentFingerPosition {
		get {
			if (SystemInfo.deviceType == DeviceType.Handheld && ControlFreak2.CF2Input.touchCount > 0)
				return ControlFreak2.CF2Input.touches [0].position;
			else
				return new Vector2 (ControlFreak2.CF2Input.mousePosition.x, ControlFreak2.CF2Input.mousePosition.y);
		}
	}
	...
	public float CenterToWorldPosition (Vector3 targetPos)
	{

		Vector3 targetScreenPos = ViewCamera.WorldToScreenPoint (targetPos);
		float targetDistance = ScreenPositionToNearestDistance (new Vector2 (targetScreenPos.x, targetScreenPos.y));

		Vector3 centerScreenPos = ViewCamera.ViewportToScreenPoint (new Vector3 (0.5f, 0.5f, 0));
		float centerDistance = ScreenPositionToNearestDistance (new Vector2 (centerScreenPos.x, centerScreenPos.y));

		float tweenDistance = Distance + centerDistance - targetDistance;
		if (snapInterval > 0)
			tweenDistance = (float)Mathf.RoundToInt (tweenDistance / snapInterval) * snapInterval;

		return TweenProgressTo (Mathf.Clamp01 (tweenDistance / MaxDistance));
	}

	public float TweenProgressTo (float progress)
	{
		distanceSpeed = 0;
		IsDragging = false;
		StopTweenProgress ();
	...
	void Update ()
	{
		...
		if (IsDragging && Interactive && !IsTweening) {
