...
namespace SgLib.Editor
{
    public static class GlobalDefineManager
    {
        ...
        {
            // TODO: This looping method currently has trouble distinguishing between BuildTargetGroup.iOS and BuildTargetGroup.iPhone,
            // because they have the same underlying value of 4, which could be casted back as
            // either iOS or iPhone; on some Unity versions (e.g. 5.4.2)
            // the result is always the latter, which is attributed as obsolete and therefore
            // will trigger the obsolete check below and exclude the platform from the job.
            foreach (BuildTargetGroup target in System.Enum.GetValues(typeof(BuildTargetGroup)))
            {
                // tvOS is not supported in 5.3.0 yet.
                #if UNITY_5_3_0
                ...
                if (target != BuildTargetGroup.Unknown && !IsBuildTargetGroupObsolete(target))
                {
        ...
        static bool IsBuildTargetGroupObsolete(BuildTargetGroup target)
        {
