    ...
    public class GameServiceManager : MonoBehaviour
    {
        ...
        static void DoReportAchievementProgress(string achievementId, double progress, Action<bool> callback)
        {
            ...
            Social.ReportProgress(
                ...
                {
                    Debug.Log(success
                        ? "Successfully reported progress of " + progress + "% for achievement: " + achievementId
                        : "Failed to report progress for achievement: " + achievementId);
        ...
        static void DoNextLoadScoreRequest()
        {
            ...
            else
            {
                ...
                if (request is { fromRank: > 0, scoreCount: > 0 })
                {
                ...
                ldb.LoadScores((bool success) =>
                {
                    ...
                    {
                        Debug.Log("Successfully loaded custom set of scores from leaderboard: " + ldb.id + ". Got " +
                                  ldb.scores.Length + " scores:");
