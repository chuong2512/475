...
public class TextureAtlasMeshGeneartor : ScriptableObject
{
		...
		private Mesh GenerateMesh (Rect textureRect, SpriteInstanceSetting instanceSetting)
		{
			...
			for (int x = 0; x < verticeCountX; x++) {
				for (int y = 0; y < verticeCountY; y++) {
					...
					uv.y = textureRect.yMax - uv.y + textureRect.yMin;
			...
			for (int planeIndexX = 0; planeIndexX < instanceSetting.tileX; planeIndexX++) {
				for (int planeIndexY = 0; planeIndexY < instanceSetting.tileY; planeIndexY++) {
					...
					int topLeftVerticeIndex = planeIndexX + planeIndexY * (instanceSetting.tileX + 1);
					int topRightVerticeIndex = topLeftVerticeIndex + 1;
					int bottomLeftVerticeIndex = topLeftVerticeIndex + (instanceSetting.tileX + 1);
	...
	public bool meshSizeByTextureRect = false;
	public Vector3 verticeOffset = Vector3.zero;
	...
	public Mesh GenerateMeshForSprite (Rect textureRect)
	{
		...
		float meshWidth = meshSizeByTextureRect ? textureRect.width : 1;
		float meshHeight = meshSizeByTextureRect ? textureRect.height : 1;
		Vector3[] vertices = new Vector3[4];
		vertices [0] = new Vector3 (0, 0, 0) * meshScale + verticeOffset;
		vertices [1] = new Vector3 (meshWidth, 0, 0) * meshScale + verticeOffset;
		vertices [2] = new Vector3 (meshWidth, -meshHeight, 0) * meshScale + verticeOffset;
		vertices [3] = new Vector3 (0, -meshHeight, 0) * meshScale + verticeOffset;
	...
	private static void SetMeshAnchorPosition (ref Mesh mesh, MeshAnchorPositions anchorPosition)
	{
		if (anchorPosition != MeshAnchorPositions.TopLeft) {
			...
			offset.x *= mesh.bounds.extents.x * 2;
			offset.y *= mesh.bounds.extents.y * 2;
