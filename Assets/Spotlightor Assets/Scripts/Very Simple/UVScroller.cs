...
{
	private static Dictionary<Material, UVScroller> singletonScrollerByMaterial = new Dictionary<Material, UVScroller> ();

	public bool scrollSharedMaterial = false;
	[HideByBooleanProperty ("scrollSharedMaterial", false)]
	...
	protected virtual void Start ()
	{
		if (scrollSharedMaterial) {
			...
			if (singleton) {
				UVScroller singletonScroller = null;
				singletonScrollerByMaterial.TryGetValue (targetMaterial, out singletonScroller);
				if (singletonScroller != null)
					Destroy (this);
				else
					singletonScrollerByMaterial [targetMaterial] = this;
			}
	...
	{
		if (scrollSharedMaterial && targetMaterial != null) {
