...
public class CharacterMotorRigidbodySafePositioner : MonoBehaviour
{
	...
	void HandleBeforeApplyMove (EnhancedCharacterController enhancedController, Vector3 totalMotion)
	{
		if (safePositionerHelper.IsColliding) {
			...
			// If CC sink into ground, after CC.Move, it'll be "bumped" onto the ground according to default CC behavior
			// If we apply safePositioner.movement again, CC will float above fast moving up platform.
			// So when CC sinked, we won't "move up" by safe positioner.
			if ((safePositionerHelper.LastCollisionFlags & CollisionFlags.Below) != 0) 
				pushMovement.y = Mathf.Min (0, pushMovement.y);
