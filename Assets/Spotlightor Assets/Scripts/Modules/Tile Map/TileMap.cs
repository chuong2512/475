...
namespace Spotlightor.TileMap
{
	public class TileMap : MonoBehaviour
	{
		public class TileMapData
		{
			...
			public bool HasTileAt (Vector3 tilePoint)
			{
				bool hasTile = false;
				if (tilePoint.x >= minX && tilePoint.x <= maxX
				    && tilePoint.z >= minZ && tilePoint.z <= maxZ) {
					int x = (int)tilePoint.x - minX;
					int z = (int)tilePoint.z - minZ;
					hasTile = tileDatas [z] [x];
				}
				return hasTile;
			}
		}
