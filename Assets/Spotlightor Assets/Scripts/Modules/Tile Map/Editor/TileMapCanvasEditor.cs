		...
		void OnSceneGUI ()
		{
			...
			for (int z = data.RowsCount - 1; z >= 0; z--) {
				...
				for (int x = 0; x < data.ColumnsCount; x++) {
					...
					if (Handles.Button (Placer.transform.TransformPoint (localPos), Quaternion.LookRotation (Placer.transform.up), Placer.tileSize.x, Placer.tileSize.x, Handles.CubeHandleCap)) {
						tileValue++;
						if (tileValue > 1)
							tileValue = 0;
