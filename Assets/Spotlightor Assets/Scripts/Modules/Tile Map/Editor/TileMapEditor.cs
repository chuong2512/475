...
namespace Spotlightor.TileMap
{
	...
	public class TileMapEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			DrawResetButton (tiles);
			DrawUpdateAllTilePositionsButton (tiles);
		...
		private void DrawLineCreateButton (List<Tile> tiles, Vector3 lineAxis, string label)
		{
			if (GUILayout.Button (label)) {
				...
				for (int i = increaseStart; i <= increaseEnd; i++) {
					...
					Vector3 checkPoint = i * increasePart + checkPart;
					if (data.HasTileAt (checkPoint))
		...
		private void DrawResetButton (List<Tile> tiles)
		{
			if (GUILayout.Button ("重置为9宫格")) {
				...
				for (int x = -1; x <= 1; x++)
					for (int z = -1; z <= 1; z++) {
						InstantiateTileAtTilePoint (new Vector3 (x, 0, z));
					}
		...
		private void DrawUpdateAllTilePositionsButton (List<Tile> tiles)
		{
